...
try:
    ...
    sys.path.append(os.path.expanduser("~/Projects/auto/carla/PythonAPI/carla"))
except IndexError:
    pass
    ...
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                if item[3] == "front_camera":
                    print(f"hud dim: {self.hud.dim}")
...
def game_loop(args):
    ...
    try:
        ...
        # model trained with https://github.com/zihaosheng/VLM-RL/blob/main/LICENSE
        # TODO: make this path a command line argument
        agent = SAC.load("../../../auto/VLM-RL/tensorboard/SAC_20251001_035126_id2/model_130000_steps.zip")
        ...
        # world.camera_manager.image
        # steer = world.player.get_control().steer
        # throttle = world.player.get_control().throttle
        # velocity = world.player.get_velocity()
        # speed = 3.6 * np.sqrt(velocity.x ** 2 + velocity.y ** 2 + velocity.z ** 2)
        # print(f"Initial state: Throttle {throttle:.2f}, Steer {steer:.2f}, Speed {speed:.2f} km/h")
        ...
        while True:
            ...
            # encoded_state for SAC VLM-RL
            image = world.camera_manager.image # (480, 640, 3)
            # image to (3, 80, 100)
            ...
            velocity = world.player.get_velocity()
            speed = 3.6 * np.sqrt(velocity.x ** 2 + velocity.y ** 2 + velocity.z ** 2)
            # print(f"Throttle {throttle:.2f}, Steer {steer:.2f}, Brake {brake:.2f}, Speed {speed:.2f} km/h")
            
            encoded_state = {}
            vehicle_measures = [steer, throttle, speed]
            encoded_state['vehicle_measures'] = vehicle_measures
            # encoded_state['rgb_camera'] = np.zeros((3, 80, 100))
            encoded_state['rgb_camera'] = image # / 255.0
            action, _ = agent.predict(encoded_state, deterministic=True)
